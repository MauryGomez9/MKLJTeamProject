...
function productsModel(db){
  let productModel = {};
  var productsCollection = db.collection("negocios");
  if ( !IndexVerified) {
  ...
  productModel.getnegocioById = (negocioId, handler)=>{
    obt.findOne({ "_id": new ObjectId(negocioId)}, (err, doc)=>{
        if(err){
          handler(err, null);
        }else{
          handler(null, doc);
  ...
  productModel.addNewnegocio = (newnegocio, handler)=>{
    obt.insertOne(newnegocio, (err, r)=>{
      if(err){
        handler(err, null);
      }else{
