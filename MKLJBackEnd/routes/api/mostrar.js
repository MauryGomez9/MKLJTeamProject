...
// json.org
var mongoModel = require('./negocios.model');
var  negociosColl = db.collection('negocios');
var  ordenColl = db.collection('negocio');
...
router.get('/byid/:negocioid', (req, res, next)=>{
  mongoModel.getnegocios(req.params.negocioid, (err, negocioDoc)=>{
    if(err){
      console.log(err);
      return res.status(500).json({"error":"Error al obtener el negocio"});
    }
    return res.status(200).json(negocioDoc);
  } );//getnegocio By Id
}); // byid:negocioid
...
router.put('/:idElemento', (req, res, next) => {
  ...
  // var modifiedObject = {};
  // var originalObject = {};
  // negociosCollection = negociosCollection.map( (e, i) => {
  //   if(e.id === id) {
